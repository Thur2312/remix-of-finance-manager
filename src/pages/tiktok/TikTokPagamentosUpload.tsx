...
function TikTokPagamentosUploadContent() {
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const orderDetailsVariants = ['order details', 'detalhes do pedido', 'detalhes dos pedidos'];
      const statementsVariants = ['statements', 'demonstrativos', 'extratos'];
      
      const orderDetailsSheetName = workbook.SheetNames.find(
        name => orderDetailsVariants.includes(name.toLowerCase().trim())
      );
      ...
      if (statementsSheetName) {
        ...
        const statementsDataAsStrings: Record<string, string>[] = statementsData.map(row => 
      ...
      if (orderDetailsSheetName) {
        const orderDetailsData: Record<string, string | number | boolean>[] = [];
        ...
        const matrix: (string | number | boolean | null)[][] = XLSX.utils.sheet_to_json(orderDetailsWs, {
        ...
        const linhasOrdersValidas = orderDetailsData.filter(row => {
          const type = row['Type'] || row['Tipo'] || row['Tipo de transação'] || '';
          const orderId = row['Order/adjustment ID'] || row['Order ID'] || row['ID do pedido'] || row['ID do pedido/ajuste'] || row['ID do pedido / ajuste'] || '';
          const typeStr = type.toString().trim().toLowerCase();
          // Aceitar tanto 'order' quanto 'pedido'
      ...
      // Convert all values to strings as required by parseAllSettlements
      const jsonData: Record<string, string>[] = orderDetailsData.map(row =>
        Object.entries(row).reduce((acc, [key, value]) => {
          acc[key] = String(value);
          return acc;
        }, {} as Record<string, string>)
      ...
      if (statementsInfo && statementsSheetName) {
        ...
        const { statements: parsedStatements } = parseStatementsSheet(statementsDataAsStrings);
...
export { TikTokPagamentosUploadContent };
