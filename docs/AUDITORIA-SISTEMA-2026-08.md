...
- [ ] **`integration-sync` cron janelado** — BLOQUEADO: `trigger_auto_sync()` (o job pg_cron) não está em migration nenhuma — precisa do usuário rodar `select pg_get_functiondef('public.trigger_auto_sync'::regprocedure);` pra reescrever com segurança. **Baixa prioridade:** `escrowBudget=150` só morde vendedor com >150 repasses/15min sustentado — nenhum cliente atual chega perto. Seletor de 90/180 dias adicionado (`e689e4e`) resolve o backfill manual, que era o uso real.
...
- [~] **`transaction_date` retroativo** — o backfill `days:180` (seletor novo) corrige; confirmar depois que o usuário rodar.
